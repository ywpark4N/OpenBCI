...
def main():
    ...
    while True:
        ...
        scale_fac_uVolts_per_count = (
            (ADS1299_Vref / (ADC_MAX)) / ADS1299_gain * 1000000.0
        )

        for adc_value in data[1]:
            # Vin 계산
            vin = adc_value * scale_fac_uVolts_per_count
            vin_values.append(vin)

        all_data = data
        all_data[1] = vin_values
        vin_values = []
        ...
        for i in range(len(all_data[1])):
            new_entry = []
            for j in range(13):
                new_entry.append(all_data[j + 1][i])
